...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        exit("USAGE: python eval_model/eval_demo.py annotation_json_file answer_json_file")
    ...
    eval.evaluate()
    acc = eval.accuracy
    print("overall: %.2f" % acc['overall'], "yes/no: %f" % acc['perAnswerType']["yes/no"],
          "number: %.2f" % acc['perAnswerType']["number"], "other: %.2f" % acc['perAnswerType']["other"])
