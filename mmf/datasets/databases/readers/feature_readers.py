...
class PaddedFasterRCNNFeatureReader:
    ...
    def read(self, image_feat_path):
        ...
        if image_info["features"].size == 1 and "features" in image_info["features"].item():
            item = image_info["features"].item()
            ...
            image_info["cls_prob"] = np.zeros((self.max_loc, item["cls_prob"].shape[1]), dtype=np.float32)
            image_info["cls_prob"][0:image_loc,] = item["cls_prob"][:self.max_loc, :]
            image_info["bbox"] = np.zeros((self.max_loc, item["bbox"].shape[1]), dtype=np.float32)
