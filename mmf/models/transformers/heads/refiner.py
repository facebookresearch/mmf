...
from faim.module.non_temporal import MLP
from mmf.modules.losses import RefinerMSLoss, RefinerContrastiveLoss
...
class Refiner(BaseTransformerHead):
    ...
    def forward(
        ...
        for modality in processed_sample_list["refiner_targets"].keys():
            ...
            elif (isinstance(self.refiner_loss, RefinerContrastiveLoss)
                    or isinstance(self.refiner_loss, RefinerMSLoss)):
                modality_targets = {}
                modality_targets["targets"] = \
                    processed_sample_list["refiner_targets"][modality]
