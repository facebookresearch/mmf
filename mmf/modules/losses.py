...
class MSLoss(nn.Module):
    ...
    def get_positive_and_negative_pairs(self, sim_vec, targets, curr_target):
        ...
        if self.is_multilabel:
            pos_pair_ = torch.masked_select(sim_vec,
                                            torch.matmul(
                                                targets,
                                                targets[0]) > 0
                                            )
        ...
        if self.is_multilabel:
            neg_pair_ = torch.masked_select(sim_vec,
                                            torch.matmul(
                                                targets,
                                                targets[0]) < 1e-5
                                            )
    ...
    def forward(self, sample_list, model_output):
        ...
        for i in range(batch_size):
            ...
            neg_pair_ = torch.masked_select(
                sim_ij,
                sim_ij > pos_similarity - self.similarity_threshold)

            # remove the pos_pair from the neg_pair list
            neg_pair_ = torch.masked_select(
                neg_pair_,
                abs(neg_pair_ - pos_similarity) > self.epsilon)
