...
def parse_arg():
    ...
    parser.add_argument('--log_dir', type=str, help="directory for log", default="/private/home/tinayujiang/logs")
...
BEST_EPOCH_ACC_REG=re.compile("best_acc(?:\s+)?:(?:\s+)?(0\.\d+).*epoch(?:\s+)?:(?:\s+)?(\d+\/\d+)")
...
def parse_config(config_file):
    ...
    eps = config['optimizer']['par']['eps'] if 'eps' in config['optimizer']['par'] else 0
    eps = str(eps)

    result =[feats,imdb ,config['data']['batch_size'], config['optimizer']['par']['lr'],eps,
            config['optimizer']['par']['weight_decay'], config['training_parameters']['lr_step'],
...
## EXTRA_ARGS='--config config/keep/baseline.yaml
...
def parse_a_job(log_dir, job_id):
    ...
    for file in glob.glob(log_dir + "/main*j" +str(job_id)+"_*out", recursive=True):
        ...
        #collect only when best_index > len of_epoch
    ...
    result = str(job_id)+"\t"+config_file_name +"\t"+cfg_info+"\t" + "%.4f" % mean +"\t"  +"%.4f" % sd+"\t"  + "%.4f" % max_val +"\t"+ str(len(bests))+"\t"+ str(complete)+"\t" +config_file +"\t"+ bests_str
    return result
...
if __name__ == "__main__":
    ...
    with open(out, 'w') as w:
        ...
        for md_idx in range(args.start, args.end+1):
            files = glob.glob(args.log_dir + "/main*j" +str(md_idx)+"_*out", recursive=True)
