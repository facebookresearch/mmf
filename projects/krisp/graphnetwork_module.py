...
def prepare_embeddings(node_names, embedding_file, add_split):
    ...
    def transform(compound_word):
        return [compound_word, "_".join([w.lower() for w in compound_word.split(" ")]), "_".join([w.capitalize() for w in compound_word.split(" ")]), 
                "-".join([w for w in compound_word.split(" ")]), "-".join([w for w in compound_word.split(" ")])]
        ...
        if add_split:
             if not found_mapping and len(node_name.split(" ")) > 1:
                ...
                if len(sub_word_vecs) == len(node_name.split(" ")):
                    ...
                    match_positions.append(np.mean(mp)) # I'm sort of ignoring match_positions except for counts
                    found_mapping = True
        else: 
...
class GraphNetworkBare(BaseModel):
    ...
    def build(self):
        ...
        if self.config.output_type in ['graph_level', 'graph_level_ansonly', 'graph_level_inputonly']:
...
class GraphNetworkModule(nn.Module):
            ...
            for ans_str in answer_vocab.word2idx_dict:  
                ...
                if config.okvqa_v_mode == "v1.0":
                    ...
                    if config.okvqa_v_mode == "v1.0-121-mc":
                        ...
                        # Try most common
                        if tx_data["v10_2_v11_mc"][ans_str] in self.name2node_idx and tx_data["v10_2_v11_mc"][ans_str] in old_graph_vocab:
                            # Get raw answer string
                            raw_ans = tx_data["v10_2_v11_mc"][ans_str]  
                        else:
                            # Otherwise try all other options
    ...
    def forward(self, sample_list):
        ...
        if self.node_features_forward is None or batch_size*self.num_nodes != self.node_features_forward.size(0): 
            ...
            # Allocate the data
            self.node_features_forward = torch.zeros(self.num_nodes * batch_size, self.in_node_dim).to(device)
            _, num_edges = self.edge_index.size()            
            self.edge_index_forward = torch.LongTensor(2, num_edges*batch_size).fill_(0).to(device)
            if self.gn.gcn_type == 'RGCN':
                self.edge_type_forward = torch.LongTensor(num_edges*batch_size).fill_(0).to(device)
        ...
        if self.gn.output_type == 'graph_level_ansonly':
            ...
            output_nodes = torch.LongTensor(all_node_idx)   # These are all non-zero nodes for the question
        else:
            output_nodes = None
        ...
        # Otherwise, proceed normally
            ...
            if "feed_vb_to_graph" in self.config_extra and self.config_extra["feed_vb_to_graph"] and self.config_extra["feed_mode"] == "feed_vb_logit_to_graph":
                ...
                vb_confs_graphindexed = torch.zeros(batch_size, self.num_nodes).to(device)
        ...
        if self.config.output_type == 'hidden_ans':
            ...
            if self.config.output_order == 'alpha':
                ...
                outputs_tmp = torch.zeros(batch_size, self.config.num_labels, self.config.node_hid_dim).to(device)
            ...
            if self.config.output_order == 'alpha':
                ...
                # Re-index into answer_vocab
                logits = torch.zeros(batch_size, self.config.num_labels).fill_(-1e3).to(device) 
...
class GraphNetwork(nn.Module):
    def __init__(self, config, in_node_dim, num_relations, num_nodes, special_input_node=False, special_input_sz=None):
        ...
        self.in_node_dim = in_node_dim  # Passed in from GraphNetworkModule which constructs the input features
        ...
        if self.output_type in ['hidden', 'hidden_subindex', 'hidden_ans']:
            ...
            self.logit_pred = nn.Linear(self.node_hid_dim, 1) # Predicts hid_dim -> 1, which then gets passed into softmax
    ...
    def forward(self, x, edge_index, edge_type=None, batch_size=1, output_nodes=None, special_node_input=None):
        ...
        if self.output_special_node or self.special_input_node:
            ...
            # Or zero-pad it         
                ...
                special_node_input = torch.zeros(batch_size, self.node_hid_dim).to(x.device)
            ...
            if self.edge_index_special is None or self.special_bs != batch_size: 
                ...
                self.edge_index_special = torch.LongTensor(spec_edges).transpose(0, 1).to(x.device)

                # Make edge type (if necessary)
                if self.gcn_type == 'RGCN':
                    self.edge_type_special = torch.LongTensor(len(spec_edges)).fill_(self.num_relations).to(x.device) # edge type is special n+1 edge type
