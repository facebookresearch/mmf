...
def one_stage_model_error_analysis(batch, myModel,answer_dict,writer):
    ...
    logit_res = myModel(input_question_variable=input_txt_variable, input_text_seq_lens=input_text_seq_lens,
    ...
    for idx, valid_answer_variable in enumerate(input_valid_answers):
        ...
        writer.write(question_id + '\t' + image_name +'\t'+ '\"'+ question+ '\"'+
                     '\t' + '\"'+ pred_ans_des+ '\"' + '\t' + str(pred_ans_count) +'\t'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_path",type=str,required=True, help="file path to model")
    ...
    data_reader_val = DataLoader(data_set_val,shuffle=False, batch_size=batch_size, collate_fn=collate_fun)
    myModel = torch.load(args.model_path)

    model_type = config['model']['model_type']
    if is_one_stageModel(model_type):
        for i, batch_data in enumerate(data_reader_val):
            one_stage_model_error_analysis(batch_data, myModel, data_set_val.answer_dict,out_writer)
    else:
        None
