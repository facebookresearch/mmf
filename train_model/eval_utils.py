...
def get_final_validation(data_set_val, batch_size,snapshot_dir,eval_model):
    final_val_data_reader = DataLoader(data_set_val, shuffle=False, batch_size=batch_size)

    files = [os.path.join(snapshot_dir, file) for file in os.listdir(snapshot_dir) if file.startswith('model')]
