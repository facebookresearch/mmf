...
from train_model.Engineer import  one_stage_run_model, compute_score_with_logits, masked_unk_softmax
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_paths", nargs='+', help="paths for model",default=None)
    parser.add_argument("--model_dirs",nargs='+',help="directories for models", default=None)
    ...
    data_set_val = prepare_eval_data_set(**config['data'], **config['model'], verbose=True)
    ...
    current_models = [] if model_files is None else [torch.load(model_file) for model_file in model_files]
    ...
    if is_one_stageModel(model_type):
        for i, batch in enumerate(data_reader_val):
            ...
            scores = compute_score_with_prob(ensembled_soft_max_result, answer_scores_np)
            total_score += scores

        print("model: %d, sample= %d, score =%.6f" % (num_of_model,total_sample, total_score / total_sample))
