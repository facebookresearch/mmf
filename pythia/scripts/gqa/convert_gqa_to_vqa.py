...
def get_imdb(file_path):
    ...
    for qid, item in tqdm.tqdm(questions.items()):
        entry = {'image_name': item['imageId'] + 'jpg',
...
def extract_image_features(image_dir, out_dir):
    extract_bbox_feats(os.path.join(image_dir, 'objects'),
                       os.path.join(out_dir, 'objects'))

    extract_spatial_feats(os.path.join(image_dir, 'spatial'),
                          os.path.join(out_dir, 'spatial'))
...
def convert_gqa_to_vqa(gqa_dir, out_dir):
    ...
    split_mapping = {'test': 'test_all_questions.json',
