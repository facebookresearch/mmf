...
class VQA2Task(BaseTask):
    ...
    def load(self, **opts):
        ...
        if self.dataset_type == 'train':
            self.dataset = self.prepare_train_data_set(**opts)
        elif self.dataset_type == 'dev':
            self.dataset = self.prepare_eval_data_set(**opts)
        elif self.dataset_type == 'test':
            self.dataset = self.prepare_test_data_set(**opts)
        else:
    ...
    def prepare_data_set(self, imdb_file_label,
        ...
        prune_filter_mod = data_config['prune_filter_module'] \
