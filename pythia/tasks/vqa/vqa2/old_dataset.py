...
from pythia.tasks.datasets.coco.coco_features_dataset import \
    COCOFeaturesDataset

from .utils import VocabDict, word_tokenize
...
def compute_answer_scores(answers, num_of_answers, unk_idx):
    ...
    for idx, answer in enumerate(answers):
        gt_answers.append({"id": idx, "answer": answer})

    for answer in set(answers):
        accs = []
        for answer_gt in gt_answers:
            other_answers = [item for item in gt_answers if item != answer_gt]
            matching_ans = [item for item in other_answers if item["answer"] == answer]
    ...
    def load_item(self, idx):
        ...
        if self.load_answer:
            ...
            #     valid_answers_tokens = iminfo['valid_answers_tokens']
                ...
                valid_answers_idx[: len(valid_answers_tokens)] = ans_idx
                ...
                valid_answers_tokens = np.random.choice(all_answers_tokens, num_tokens)
                ans_idx = [
                    self.answer_dict.word2idx(word_tokenize(ans))
                    for ans in valid_answers_tokens
                ]
...
class VQAConcatDataset(ConcatDataset):
    ...
    def calculate_loss_and_metrics(self, output, expected_output, info):
        return self.datasets[0].calculate_loss_and_metrics(
            output, expected_output, info
        )
