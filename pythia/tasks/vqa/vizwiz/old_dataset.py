...
class VizWizDataset(VQA2Dataset):
    def __init__(self, imdb_file, image_feat_directories, verbose=False, **data_params):
        ...
        if "use_ocr" not in self.data_params:
            self.writer.write(
                "use_ocr configuration not present. " "Setting to False.", "warning"
            )
            self.use_ocr = False
        ...
        if "copy_type" not in self.data_params:
            self.writer.write(
                "copy_type configuration is not present. " "Setting to False", "warning"
            )
        ...
        if "copy_included" not in self.data_params:
            self.writer.write(
                "copy_included configuration not present. " "Setting to False",
                "warning",
            )
        ...
        if self.use_ocr:
            ...
            self.default_tokens = ["<pad>" for _ in range(self.context_max_len)]

            if self.context_vocab.type == "model":
                self.context_seq_shape = (
        ...
        if self.copy_type == "soft":
            self.writer.write("use_soft_copy is enabled, loading" " sym spell checker")
    ...
    def _init_spell_checker(self):
        ...
        if "vocab_spell_checker" in self.data_params:
            ...
            if not os.path.exists(vocab_path):
                self.writer.write(
                    "Vocab for spell checker, doesn't" " exist at %s" % vocab_path,
                    "error",
                )
                return
            ...
            self.spell_checker = SymSpell(
                self.SPELL_CHECKER_INITIAL_CAPACITY,
    ...
    def prepare_batch(self, batch):
        ...
        if "order_vectors" in batch:
            ...
            order_vectors = Variable(order_vectors, requires_grad=False, volatile=False)
    ...
    def load_item(self, idx):
        ...
        if self.use_ocr:
            ...
            if self.use_spell_checker:
                spell_checked = self.spell_checker.lookup_compound(" ".join(tokens), 2)
            ...
            token_idxs = [
                np.mean([self.context_vocab.stoi[w] for w in word.split(" ")], axis=0)
                for word in final_tokens
            ]

            order_vectors = np.zeros(
                (self.context_max_len, self.context_max_len), np.float32
            )
            ...
            if self.use_ocr_info:
                sample["contexts"] = np.concatenate(
                    [sample["contexts"], ocr_infos], axis=-1
                )
            ...
            if attention_supervision_org is not None:
                attention_supervision = np.zeros((self.context_max_len), np.float)
                attention_supervision[:original_len] = attention_supervision_org[
                    :original_len
                ]
    ...
    def _update_based_on_copy(self, idx, sample):
        ...
        if self.load_answer and "answers" in iminfo:
            ...
            for idx, token in enumerate(answers):
                ...
                gt_answers.append({"id": idx, "answer": token})
            ...
            for idx, token in enumerate(sample["ocr_tokens"][:context_len]):
                ...
                for answer_gt in gt_answers:
                    other_answers = [item for item in gt_answers if item != answer_gt]
                    matching_ans = [
                        item for item in other_answers if item["answer"] == token
                    ]
        ...
        answer_scores_expanded[: self.answer_space_size] = answer_scores
    ...
    def init_args(self, parser):
        ...
        parser.add_argument(
            ...
            type=bool,
            default=None,
            help="If attention supervision loss should" " be used",
        )
