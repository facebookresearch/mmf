...
from top_down_bottom_up.top_down_bottom_up_model import top_down_bottom_up_model
import numpy as np
...
class Test_top_down_bottom_up_model(unittest.TestCase):
    def test_classifier(self):
        ...
        my_classifier = logit_classifier(joint_embedding_dim, num_ans_candidates, image_embedding_dim, text_embeding_dim)
    ...
    def test_question_embedding(self):
        ...
        batch_size = 32
        question_len = 10
        my_word_embedding_model = QuestionEmbeding(num_vocab, embedding_dim, lstm_dim, lstm_layer, dropout, batch_first)
        my_word_embedding_model = my_word_embedding_model.cuda() if use_cuda else my_word_embedding_model
        input_txt = Variable(torch.rand(batch_size, question_len).type(torch.LongTensor) %num_vocab)
    ...
    def test_image_embedding(self):
        ...
        num_of_loc = 5
        batch_size = 16
        my_image_embeding = image_embedding(image_feat_dim, txt_embedding_dim, hidden_size)
    ...
    def test_model(self):
        ...
        question_embedding_model = QuestionEmbeding(num_vocab, txt_embedding_dim, lstm_dim,
                                                    lstm_layer=2, dropout=0.1, batch_first=batch_first)
        my_classifier = logit_classifier(joint_embedding_dim, num_ans_candidates, image_feat_dim,
                                         txt_embedding_dim)
        loss = torch.nn.CrossEntropyLoss()

        my_model = top_down_bottom_up_model(image_embedding_model, question_embedding_model, my_classifier,loss)
        #image_feat = Variable(torch.randn(batch_size, num_of_loc, image_feat_dim))
