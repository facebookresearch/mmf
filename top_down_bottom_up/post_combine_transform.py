...
class LinearTransform(nn.Module):
    def __init__(self, in_dim, **kwargs):
        ...
        self.lc = weight_norm(nn.Linear(in_features=in_dim, out_features=kwargs['out_dim']),dim=None)
...
class ConvTransform(nn.Module):
    def __init__(self, in_dim, **kwargs):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_dim, out_channels=kwargs['hidden_dim'], kernel_size=1)
        self.conv2 = nn.Conv2d(in_channels=kwargs['hidden_dim'], out_channels=kwargs['out_dim'], kernel_size=1)
        self.out_dim = kwargs['out_dim']

    def forward(self, x):
        if len(x.size()) == 3: ## N x k xdim
            x_reshape = torch.unsqueeze(x.permute(0,2,1),3) # N x dim x k x 1
        elif len(x.size()) == 2: ## N x dim
            x_reshape = torch.unsqueeze(torch.unsqueeze(x, 2),3) # N x dim x 1 x 1
