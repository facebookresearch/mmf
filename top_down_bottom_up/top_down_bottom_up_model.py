...
class vqa_multi_modal_model(nn.Module):
    def __init__(self, image_embedding_models_list, question_embedding_models, multi_modal_combine, classifier, image_feature_encode_list, inter_model=None):
    ...
    def forward(self, image_feat_variables, input_question_variable, image_dim_variable,input_answers=None, **kwargs):
        question_embeddings = []
        for q_model in self.question_embedding_models:
            q_embedding = q_model(input_question_variable)
            question_embeddings.append(q_embedding)
        ...
        assert (len(image_feat_variables) == len(self.image_feature_encode_list)), \
            "number of image feature model doesnot equal to number of image features"
        ...
        joint_embedding = self.multi_modal_combine(image_embedding_total, question_embedding_total)
        logit_res = self.classifier(joint_embedding)
